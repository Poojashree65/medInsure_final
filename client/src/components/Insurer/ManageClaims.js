...
function fuzzyContains(text, word) {
  ...
  // Check each word in text against target word with 60% similarity
...
function calcScore(results) {
  ...
  const weights = { documentType:15, duplicate:20, patientName:20, hospitalName:15, claimAmount:20, treatmentDate:10 };
...
function ManageClaims({ account, web3 }) {
  ...
  const runDocumentVerification = async (claim, imageFile) => {

    setVerifying(true); setVerificationResults(null); setExtractedText(""); setOcrProgress(0);
    ...
    try {
      ...
      r.documentType = found.length >= 2 ? { status:"pass", label:"Document Type", detail:"Medical document detected (" + found.slice(0,4).join(", ") + ")" } : { status:"warning", label:"Document Type", detail:"Only " + found.length + " medical keyword(s) found" };

      // Patient name — OCR match
      ...
      const nameFound = pFound.length > 0 || fuzzyContains(text, firstName);

      r.patientName = nameFound
      ...
      // Claim amount — match ETH value directly
      ...
      // Treatment date — check both YYYY-MM-DD and DD/MM/YYYY
  ...
  const getOverallStatus = (r) => { if (!r) return "pending"; const vals = Object.values(r); if (vals.some(v => v.status === "fail")) return "fail"; if (vals.every(v => v.status === "pass")) return "pass"; return "warning"; };
            ...
            <div className="mc-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="mc-modal-body">
                ...
                {selected.status === "Pending" && (
                  ...
                  <div style={{marginTop:"16px",border:"1px solid #dde3ef",borderRadius:"10px",overflow:"hidden"}}>
                    ...
                    <div style={{background:"#f4f7fc",padding:"12px 16px",borderBottom:"1px solid #dde3ef",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:"13px",fontWeight:"700",color:"#0d1b35"}}>Document Verification</div>

                        <div style={{fontSize:"11px",color:"#7a8aa8",marginTop:"2px"}}>Automatically fetch and verify the document uploaded by the hospital from IPFS</div>
                      ...
                      {verificationResults && verifyClaimId?.toString()===selected.claimId?.toString() && (
                        ...
                        <span style={{fontSize:"11px",fontWeight:"700",padding:"4px 10px",borderRadius:"20px",
                          ...
                          background:getOverallStatus(verificationResults)==="pass"?"#e8f5e9":getOverallStatus(verificationResults)==="fail"?"#ffebee":"#fff8e1",

                          color:getOverallStatus(verificationResults)==="pass"?"#2e7d32":getOverallStatus(verificationResults)==="fail"?"#c62828":"#e65100",

                          border:`1px solid ${getOverallStatus(verificationResults)==="pass"?"#a5d6a7":getOverallStatus(verificationResults)==="fail"?"#ef9a9a":"#ffe082"}`}}>

                          {getOverallStatus(verificationResults)==="pass"?"✓ All Verified":getOverallStatus(verificationResults)==="fail"?"✗ Failed":"⚠️ Manual Review"}
                    ...
                    <div style={{padding:"14px 16px"}}>
                      ...
                      {/* Verify button — fetches from IPFS automatically */}

                      <button

                        style={{background:"#1565c0",color:"#fff",border:"none",padding:"9px 20px",borderRadius:"6px",fontSize:"12px",fontWeight:"700",cursor:verifying?"not-allowed":"pointer",opacity:verifying?0.7:1,fontFamily:"inherit",marginBottom:"12px"}}

                        disabled={verifying}

                        onClick={async () => {

                          if (!selected.ipfsCID) { alert("No IPFS document found for this claim."); return; }

                          try {

                            const url = `https://gateway.pinata.cloud/ipfs/${selected.ipfsCID}`;

                            const res = await fetch(url);

                            const blob = await res.blob();

                            const file = new File([blob], "claim_document.jpg", { type: blob.type || "image/jpeg" });

                            runDocumentVerification(selected, file);

                          } catch {

                            // fallback — try public IPFS gateway

                            try {

                              const url2 = `https://ipfs.io/ipfs/${selected.ipfsCID}`;

                              const res2 = await fetch(url2);

                              const blob2 = await res2.blob();

                              const file2 = new File([blob2], "claim_document.jpg", { type: blob2.type || "image/jpeg" });

                              runDocumentVerification(selected, file2);

                            } catch { alert("Could not fetch document from IPFS. Check your connection."); }
                      ...
                      {verifying && verifyClaimId?.toString()===selected.claimId?.toString() && (
                        ...
                        <div style={{marginBottom:"12px"}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",fontSize:"11px",color:"#7a8aa8",marginBottom:"4px"}}>

                            <span>OCR Processing...</span><span>{ocrProgress}%</span>

                          </div>

                          <div style={{height:"6px",background:"#eef3fb",borderRadius:"3px",overflow:"hidden"}}>

                            <div style={{height:"100%",background:"#1565c0",borderRadius:"3px",width:`${ocrProgress}%`,transition:"width 0.3s"}}/>
                        ...
                        <>
                          ...
                          {(() => {
                            ...
                            return (
                              ...
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",borderRadius:"8px",background:rec.bg,border:"1.5px solid "+rec.border,marginBottom:"12px"}}>

                                <div>

                                  <div style={{fontSize:"13px",fontWeight:"800",color:rec.color}}>{rec.text}</div>

                                  <div style={{fontSize:"11px",color:"#7a8aa8",marginTop:"2px"}}>Based on 6 automated checks</div>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"12px"}}>
                            ...
                            {Object.values(verificationResults).map((v,i) => (
                              ...
                              <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"8px",padding:"10px 12px",borderRadius:"7px",background:sBg(v.status),border:`1px solid ${sBorder(v.status)}`}}>

                                <div style={{width:"20px",height:"20px",borderRadius:"50%",background:sColor(v.status),color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:"900",flexShrink:0}}>{sIcon(v.status)}</div>

                                <div>

                                  <div style={{fontSize:"11px",fontWeight:"700",color:"#0d1b35"}}>{v.label}</div>

                                  <div style={{fontSize:"10px",color:sColor(v.status),marginTop:"2px",lineHeight:1.4}}>{v.detail}</div>
